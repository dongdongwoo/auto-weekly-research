...
**1시간마다** 오늘(KST) 뉴스를 증분 수집하고, **데일리/주간 급등 스냅샷**을 Notion에 저장합니다. **주간 인사이트**는 **월요일 KST 09:00**에 **지난주** 일일을 종합·검증 후 생성합니다. 노션은 저장소, **웹 대시보드(Vercel)** 는 Notion 읽기 전용입니다.
...
| 명령              | 설명 |
| ----------------- | ---- |
| `npm run hourly`  | **자동 실행용.** 오늘 증분 수집 + 트렌드 스냅샷. **월요일** KST 09:00에 주간 인사이트. KST 00시면 새 데일리 토글 생성 |
...
| **월요일 KST 09:00** | **지난주(월~일) 일일** 종합 → 그 주(W37 등) **주간 인사이트** 생성 |
| KST 10:00–23:00 | 오늘 증분 수집 → 트렌드 스냅샷 (주간 인사이트는 유지) |

월요일 00시면 새 ISO 주 페이지도 함께 열립니다.

주간 인사이트는 **매주 월요일 09:00**에 **지난주** 일일 리서치를 모아 만들고, 검증 단계에서 수치·날짜·당사자를 웹 검색으로 교차확인합니다. 이번 주(월~일)는 일일만 쌓입니다.
