...
- **축당 1건만 고르지 말 것.** 해당 날짜에 관련 뉴스·발표·데이터가 여러 건이면 **각각 별도 항목**으로 모두 포함
- 중요도 순으로 정렬하되, 확인된 소식은 빠짐없이 수집 (최대 5건/축)
- 하루 전체 기준 **총 15~25건** 목표 (소식이 적은 날은 그만큼만)
${AXES}

## 출력 형식 (마크다운)
- 맨 위에 "## ${newsDateIso} 일일 리서치" 헤딩
- 축별 "## 축 이름" 헤딩 아래 불릿 (**해당 축 뉴스가 여러 건이면 여러 불릿**)
- 각 건:
  - **[헤드라인]** — 무슨 일인지 1~2문장
  - **분석** — 왜 중요한지, 업계에 어떤 의미인지 1~2문장
  - **출처**: [매체명](URL)
- 신규 소식이 전혀 없으면 "## 요약"에 "신규 항목 없음" 한 줄만
- **"## 참고 출처" 섹션은 만들지 말 것** (각 항목 출처 줄에 링크 있음)
${LINK_RULE}
- 다른 인사말 없이 본문만 출력`;
}
