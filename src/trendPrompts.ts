...
export function dailiesLogsToMarkdown(logs: { iso: string; content: string }[]): string {
  return logs
    .map((d) => {
...
- **3~5개**만, 파급 큰 순 (**제목 앞에 1· 2· 같은 순번 붙이지 마**)
...
## 작성 규칙
- 비슷한 사건은 **하나의 이슈**로 묶어라
- **3~5개**만, 기간 내 파급 큰 순 (**제목 앞에 1· 2· 같은 순번 붙이지 마**)
